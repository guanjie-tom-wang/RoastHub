...
import com.tomwang.roasthub.service.MississaugaService;
...
@RestController
@RequestMapping("/mississauga")
public class MississaugaController {
    @Autowired
    ...
    private static final Logger logger = LoggerFactory.getLogger(MississaugaController.class);

    @Autowired
    public MississaugaController(MississaugaService mississaugaService) {
        this.mississaugaService = mississaugaService;
    }
    @PostMapping("/upload")
    public ResponseEntity<?> uploadFile(
        ...
        // 用户已验证，继续上传逻辑
        try {
            List<String> fileIds = mississaugaService.storeFiles(image, file, name, type, user.getUsername());
    ...
    public List<GridFSFile> listFiles() {
        return mississaugaService.listFiles();
    }
    ...
    public ResponseEntity<Resource> downloadFile(@PathVariable String id) {
        ...
        try {
            ...
            if (fileContent == null) {
                GridFSFile gridFSFile = mississaugaService.getFile(new ObjectId(id));
                if (gridFSFile == null) {
                ...
                mississaugaService.downloadStream(gridFSFile.getObjectId(), outputStream);
    ...
    public ResponseEntity<List<RecipeItem>> getBreakfastItems() {
        List<GridFSFile> files = mississaugaService.getRandomFiles(1000);
        ...
        for (GridFSFile file : files) {
            ...
            ObjectId pdfId = mississaugaService.findPdfIdByName(itemName);
            ObjectId imageId = mississaugaService.findImageIdByName(itemName);
