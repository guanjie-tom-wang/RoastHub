...
import com.tomwang.roasthub.service.KitchenerService;
...
public class KitchenerController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(KitchenerController.class);

    @Autowired
    public KitchenerController(KitchenerService kitchenerService) {
    ...
    public ResponseEntity<?> uploadFile(
        ...
        // 用户已验证，继续上传逻辑
        try {
            List<String> fileIds = kitchenerService.storeFiles(image, file, name, type, user.getUsername());
    ...
    public List<GridFSFile> listFiles() {
        return kitchenerService.listFiles();
    }
    ...
    public ResponseEntity<List<RecipeItem>> getBreakfastItems() {
        List<GridFSFile> files = kitchenerService.getRandomFiles(1000);
        ...
        for (GridFSFile file : files) {
            ...
            ObjectId pdfId = kitchenerService.findPdfIdByName(itemName);
            ObjectId imageId = kitchenerService.findImageIdByName(itemName);
